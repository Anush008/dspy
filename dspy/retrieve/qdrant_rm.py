...
class QdrantRM(dspy.Retrieve):
    ...
    def __init__(
        ...
        Args:
            ...
            vector_name (str, optional): Name of the vector in the collection. Default: Th first available vector name.
        """
        self._qdrant_collection_name = qdrant_collection_name
        self._qdrant_client = qdrant_client

        self._vectorizer = vectorizer or FastEmbedVectorizer(qdrant_client.embedding_model_name)
    ...
    def forward(self, query_or_queries: Union[str, list[str]], k: Optional[int] = None, **kwargs) -> dspy.Prediction:
        ...
        batch_results = self._qdrant_client.search_batch(self._qdrant_collection_name, requests=search_requests)
    ...
    def _get_first_vector_name(self) -> str | None:
        vectors = self._qdrant_client.get_collection(self._qdrant_collection_name).config.params.vectors
